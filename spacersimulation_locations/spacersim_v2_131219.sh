...
for i in {1..1000}
do
python spacersimulation_v2/random_spacer_sampling_fcl2_v2.py --genome fcl2.fasta --mode random -c 430 -ps NNNNNTAAA -pp downstream > /dev/null 2>&1
cd-hit-est-2d -i $c1_pool_cluster -i2 randomizedSpacers.fasta -o spacersimulation_v2/clust.clstr -c 0.90 -M 16000 -T 8 > /dev/null 2>&1
...
clusters=$(grep ">" -c spacersimulation_v2/clust.clstr)
#echo $clusters
...
printf "${clusteredList[*]}\n" > spacersimulation_v2/random_set.txt
...
for i in {1..1000}
do
python spacersimulation_v2/random_spacer_sampling_fcl2_v2.py --genome fcl2.fasta --mode PAM -c 430 -ps NNNNNTAAA -pp downstream > /dev/null 2>&1
cd-hit-est-2d -i $c1_pool_cluster -i2 randomizedSpacers.fasta -o spacersimulation_v2/clust.clstr -c 0.90 -M 16000 -T 8 > /dev/null 2>&1
#cd-hit-est-2d -i ../SAM/CorrectedSpacers/B+P_b_+1_C1_mapped_fcl2_C1.fasta -i2 randomizedSpacers.fasta -o clust.clstr -c 0.8 -M 16000 -T 8 > /dev/null 2>&1
clusters=$(grep ">" -c spacersimulation_v2/clust.clstr)
#echo $clusters
...
printf "${clusteredList[*]}\n" > spacersimulation_v2/PAM_set.txt
